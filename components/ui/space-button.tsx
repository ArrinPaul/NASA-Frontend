  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-sm font-medium transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        space:
          "bg-gradient-to-r from-primary to-accent text-primary-foreground shadow-lg hover:shadow-xl hover:shadow-primary/30 hover:-translate-y-1 glow-primary",
        orbit:
          "border-2 border-dashed border-primary/30 bg-background/50 backdrop-blur-sm hover:border-primary hover:bg-primary/10 hover:glow-primary",
      },
      size: {
...
const SpaceButton = React.forwardRef<HTMLButtonElement, SpaceButtonProps>(
  ({ className, variant, size, asChild = false, ...props }, ref) => {
    ...
    return <Comp className={cn(spaceButtonVariants({ variant, size, className }))} ref={ref} {...props} />
  },
)
